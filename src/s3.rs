...
pub(crate) async fn expand_s3_dir(s3_uri: &PathBuf) -> Result<Vec<PathBuf>> {
    ...
    if let Some(filename) = s3_uri.file_name() {
        if filename.to_string_lossy().ends_with(".jsonl.gz")
            || filename.to_string_lossy().ends_with(".json.gz")
            || filename.to_string_lossy().ends_with(".jsonl.zstd")
        {
            s3_files.push(s3_uri.to_path_buf());
            return Ok(s3_files);
        }
    }
    ...
    while let Some(result) = response.next().await {
        match result {
            ...
            Err(err) => {
                bail!("Error collecting S3 files: {err:?}")
            }
        }
    }
...
pub(crate) async fn get_object_with_retry(
    bucket: &str,
    key: &str,
...
pub(crate) async fn get_reader_from_s3<P: AsRef<Path> + std::fmt::Debug>(
    path: P,
    num_retries: Option<usize>,
    ...
    if path.as_ref().extension().unwrap() == "zstd" {
        ...
        reader
            ...
            .await
            .expect(&format!("Failed to read data from {path:?}"));
    } else {
